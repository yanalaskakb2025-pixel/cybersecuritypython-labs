...
class ValidationError(Exception): # Власний клас винятків, який успадковується від базового класу Exception
    """Виняток для помилок валідації пароля."""

MIN_PASSWORD_LENGTH = 10
SALT = f"{VARIANT:0>5}"  # Сіль для хешування

DATA_DIR = os.path.join(os.path.dirname(__file__), "data")  # Динамічне формування шляхів до папки data і файлів users.csv,log.json
...
def generate_hash(password: str, salt: str = "00000") -> str:   # Оголошення ф-ції генерації хешу  
    if password is None or salt is None or password == "" or salt == "":  # Перевірка чи пароль або сіль відсутні або є порожніми рядками
    ...
    data_to_hash = (password + salt).encode("utf-8")  # Конкатинація паролю і солі і кодування рядка в байтову послідовність 
    return hashlib.sha3_224(data_to_hash).hexdigest() # Обчислення хешу за допомогою SHA3-224 і повернення його у вигляді шістнадцяткового рядка

def create_user(username: str, password: str) -> tuple[str, str]:  # Викликає користувача з сіллю і повертає логін з хешем
    if not username or not username.strip(): # Якщо логін порожній 
        raise ValueError("Помилка!!! Логін не може бути пустим")
    if not password or not password.strip(): # Якщо порожній пароль
            raise ValueError("Помилка!!! Пароль не може бути порожнім")
    hash_value = generate_hash(password, salt=SALT)
    
    return username, hash_value

def create_users(users_list: tuple[tuple[str, str], ...]) -> None: # Реєстрація користувачів з попереднього списку
    os.makedirs(DATA_DIR, exist_ok=True) # Створення папки data

    valid_users = [] 
    for username, password in users_list:  # Цикл проходиться по кожному користувачу і перехоплює некоректні паролі
        try:
    ...
    try:
        with open(CSV_FILE_PATH, mode="w", newline="", encoding="utf-8") as f: # Запис успішно зареєстрованих користувачів
            writer = csv.writer(f)
...
def read_users_db() -> list[tuple[str, str]]: # Зчитування користувачів з CSV_FILE_PATH та виведення їх у термінал.
    users_db = []
    try:
        with open(CSV_FILE_PATH, mode="r", encoding="utf-8") as f:  # Відкриває файл для читання
            reader = csv.reader(f)
            header = next(reader, None)  # Пропускає заголовок
            if header: 
                for row in reader:  # Проходить по кожному рядку і заповнює список users_db
                    if row:
...
def log_event(func): # Перехоплює виклики логів
    ...
    def wrapper(username: str, password: str, *args, **kwargs):  
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")  # Фіксація поточного часу та встановлення статусу спроби входу
        ...
        try:  # Виконання ф-ції логін, якщо повертає True то статус змінюється на success, якщо ні то failure
            ...
            logs = []  
            if os.path.exists(LOG_FILE_PATH):  # Зчитує існуючий файл JSON логів і додає новий запис логів у список
                try:
...
def login(username: str, password: str) -> bool:  # Оголошення автентифікації
    ...
    try:
        input_hash = generate_hash(password, salt=SALT)  # Хешування введеного пароля з сіллю
...
def main():
    ...
    for uname, pwd in test_cases:  # Проходить по всіх кейсах
        try:
            ...
            status_str = "Успішно" if success else "Відмовлено"
    ...
    print(f"\n[Успіх] Логи збережено у {LOG_FILE_PATH}")
